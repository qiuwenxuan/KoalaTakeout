			...
			<view>
				<view class="box" v-for="item in ordersList" :key="item.id" style="margin-bottom: 10rpx">
					...
					<view style="display: flex; align-items: center; grid-gap: 20rpx; margin-bottom: 10rpx" @click="goOrdersItem(item.id)">
						...
						<view>
							实付￥
							<text style="font-size: 36rpx; font-weight: bold; color: red">{{ item.actual }}</text>
						</view>
					</view>
					<view style="display: flex; min-height: 40rpx">
						...
						<view style="flex: 1; text-align: right">
							...
							<uni-tag
								...
								@click="changeStatus(item, '已退款')"
							></uni-tag>
							<uni-tag v-if="item.status === '待收货'" text="确认收货" size="large" type="warning" @click="changeStatus(item, '待评价')"></uni-tag>
							<uni-tag v-if="item.status === '待评价'" text="发布评价" size="large" type="warning" @click="goComment(item.id)"></uni-tag>
						</view>
					</view>
				</view>
			</view>
