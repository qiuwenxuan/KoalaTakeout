<template>
	<view style="padding: 20rpx">
		...
		<view class="box" style="margin: 20rpx 0">
			<navigator :url="'/pages/detail/detail?businessId=' + orders.businessId" style="margin-bottom: 20rpx; font-size: 36rpx; font-weight: bold; color: #666">
				{{ orders.businessName }}
			</navigator>
			...
			<view>
				<view style="display: flex; grid-gap: 20rpx; margin-bottom: 20rpx" v-for="item in ordersItemList" :key="item.id">
					...
					<view style="width: 150rpx; text-align: right; color: red; padding-top: 10rpx">￥{{ item.price }}</view>
				</view>
			</view>
			<!-- 显示优惠和总金额信息 -->
			...
			<view style="flex: 1; text-align: right; margin-top: 20rpx">
				<uni-tag v-if="orders.status === '待支付'" text="支付" size="large" type="primary" @click="changeStatus(item, '待发货')"></uni-tag>
				<uni-tag v-if="orders.status === '待发货'" text="申请退款" size="large" type="error" @click="changeStatus(item, '已退款')"></uni-tag>
				<uni-tag v-if="orders.status === '待收货'" text="确认收货" size="large" type="warning" @click="changeStatus(item, '待评价')"></uni-tag>
				<uni-tag v-if="orders.status === '待评价'" text="评价" size="large" type="royal"></uni-tag>
			</view>
		</view>
		<!-- 商品和金额信息 -->
	</view>
